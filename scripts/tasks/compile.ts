...
import { writeFileSync, existsSync, mkdirSync, readFileSync, readdirSync, renameSync, chmodSync } from 'fs';
...
export function compilePDF(config: ReleaseConfig): void {
  ...
  const texFlags = '-synctex=0 -interaction=nonstopmode -output-directory=build';
  const pdflatex = `pdflatex ${texFlags} ${sourceTex}`;

  // Write Docker commands to a shell script to avoid shell escaping issues
  // with execSync/spawnSync command string interpretation.
...
`;
  ...
  const scriptPath = 'build/compile.sh';
  writeFileSync(scriptPath, scriptContent);
  chmodSync(scriptPath, 0o755);

  console.log('  Running LaTeX compilation pipeline in Docker...');
  ...
  if (result.status !== 0) {
    throw new Error(
      `Docker compilation script failed with exit code ${result.status}.\n` +
      `Check build/main.log or build/main.blg for details.`
    );
  }
  ...
  if (!existsSync(sourcePdf)) {
    throw new Error(
      ...
      `Docker exit code: ${result.status}\n` +
      `Check build/main.log for details.`
    );
  }
